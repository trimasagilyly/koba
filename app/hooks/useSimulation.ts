...
import { computeAgentWealthShares } from "../utils/computeAgentWealthShares";
import { createAgents } from "../utils/createAgents";
import { seedAgentWealth } from "../utils/seedAgentWealth";
...
export function useSimulation(
  ...
  const initSim = useCallback(() => {
    ...
    seedAgentWealth(agents, numStocks);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (tickRef.current % 10 === 0) {
        ...
        setWealthByType(computeAgentWealthShares(agents, currentPrices));
